...
enum ConnectedAccountError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .accountNotFound(let id):
            return "Connected account not found: \(id)"
        case .tokenNotFound(let id):
            return "OAuth token not found for account: \(id)"
        case .tokenDecodingFailed(let error):
            return "Failed to decode OAuth token: \(error.localizedDescription)"
        case .tokenStorageFailed(let error):
            return "Failed to store OAuth token: \(error.localizedDescription)"
        case .noRefreshToken(let id):
            return "No refresh token available for account: \(id). Re-authentication required."
        case .refreshNotSupported(let provider):
            return "Token refresh not supported for \(provider.displayName)"
        case .tokenRefreshFailed(let provider):
            return "Failed to refresh \(provider.displayName) token"
